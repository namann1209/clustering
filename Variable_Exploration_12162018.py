...
#User input 
keyword_to_filter_column_description = ["Race and Ethnicity", "Income Race and Ethnicity", "Business Sector", "Average Expenditure Alcoholic", "Percent Tenure", "Percent Labor Force", "Sports Events - Attend", "Sports Events - Listen", "Dining"]  
off_premise_channels = ['LIQUOR','CONVENIENCE','GROCERY', 'MASS MERCH']
columns_to_be_dropped = []
#Prepared only after studying the results of correaltion matrix to remove variables based on business sense
correlation_matrix_columns = ['XCYA10V001', 'XCYA12V009', 'CYEC17V001', 'PYCYPOPGRW', 'CYB26VV01', 'CYPOPDENS', 'CYA01V001', 'XCYB01V002']
#Columns for which no bucketing is required
column_no_bucketing_required = ['XCYFEM','XCYMALE','XCYA10V001', 'XCYA12V009', 'CYEC17V001', 'PYCYPOPGRW', 'CYB26VV01', 'CYPOPDENS', 'CYB03V001', 'CYB02V001', 'CYA01V001', 'XCX02V194', 'XCX02V160', 'CYB17MED', 'CYB21VBASE', 'XCYB01V002', 'XCYB01V003', 'SMFAM007P_PCT']
#Description columns
descriptive_columns = ['rtlr_party_id','WSLR_NBR', 'WSLR_ASGND_CUST_NBR', 'ZIP', 'CHANNEL', 'Urbanicity']
...
def data_ingestion():
    ...
    active_retailer_vol = sql_execute("SELECT rtlr_party_id, zip, channel, brnd_cd, sum(sales_bbls) as sales_bbls \
      FROM [zip_analytics_test].[str_sales_extract_all] where [CAL_YR_MO_NBR] > 201709  \
      group by rtlr_party_id, zip, brnd_cd, channel")
    ...
    #data_dictionary = pd.read_csv("Varibale names 2018.csv")
    #Selecting Name and Description columns from data dictionary
...
def Correlation_of_demo_columns_description(dataframe_PCA_non_PCA):
    ...
    ##Converting correaltion file to long format
...
def PCR(x):
    if (str(x)=='alteryx'):
        ...
        #Clubbing Mass Merc with grocery. Hence replacing Mass Merc with Grocery
        ...
        pca_results = map(segmentation, keyword_to_filter_column_description) 
        
        PCA_Dataframes = pd.concat(pca_results, axis=1)
        PCA_Dataframes = PCA_Dataframes.reset_index(drop = True)
        ...
        dataframe_PCA_non_PCA = pd.concat([demographics_off[['rtlr_party_id']].reset_index(drop=True), Dataframe_Non_PCA, PCA_Dataframes], axis = 1)             
        dataframe_PCA_non_PCA = dataframe_PCA_non_PCA.reset_index(drop = True)
        ...
        print("Only supports alteryx data currently")

######Calling PCR function
if '__name_'== '__main__':
    #
    important_features  = PCR('Alteryx')
