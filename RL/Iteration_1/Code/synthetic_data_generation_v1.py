...
recomm_df['sales_exec'] = None

for brnd in recomm_df.brnd_cd.unique():
    if brnd in sale_rep_fav:
        p = 0.6
    else:
        p = 0.6
    size = recomm_df[recomm_df.brnd_cd==brnd].shape[0]
    recomm_df.loc[recomm_df.brnd_cd==brnd, 'sales_exec'] = np.random.choice([0, 1], size=size, p=[1-p, p])
                
recomm_df['sales_exec'] = recomm_df['brnd_cd'].apply(lambda x: 1 if x in sale_rep_fav else 0)

recomm_df.to_csv(os.path.join(filepath, 'recomm_exec_data_v5.csv'), index=False)
...
recomm_df.to_csv(os.path.join(filepath, 'sales_exec_5yr_dummy_data_v5.csv'), 
                 index=False)
