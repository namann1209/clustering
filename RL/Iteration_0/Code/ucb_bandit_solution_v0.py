...
def ucb_bandit(num_occur, avg_occur, round_t):
    return (avg_occur + np.sqrt((np.log(round_t))/num_occur))      
...
if __name__ == "__main__":
    ...
    # read raw data
    recom_exec_df = pd.read_csv(os.path.join(filepath, 'recomm_exec_data_v0.csv'))
    ...
    final_df.to_csv(os.path.join(filepath, 'new_recommendations_v0.csv'), 
                    index=False)
