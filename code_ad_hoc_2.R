...
cluster_df=dbGetQuery(con,"select distinct rtlr_num, segment from zip_analytics_test.rtlr_num_clusters_without_adjustment_for_coli")
...
############################################################
# mult=dbGetQuery(con, "select state state_cd, sum(total_volume_sales_case) vol_cases, cal_yr_mo_nbr, brewer, segment from (select state, total_volume_sales_case , iso_yr_wk_nbr, brewer_nm brewer,
#                         case
#                 when ab_subseg='PREMIUM PLUS' then 'CORE PLUS'
#                 when ab_subseg='CORE IMPORT' then 'CORE PLUS'
#                 when ab_seg='HE BEER' then 'Premium'
#                 when ab_seg='HE NEAR BEER' then 'FMB'
#                 when ab_seg='NEAR BEER' then 'FMB'
#                 else ab_seg
#                 end as segment from zip_analytics_test.iri_master where iso_yr_wk_nbr>201700) as a inner join (select distinct iso_yr_wk_nbr, cal_yr_mo_nbr from zip_analytics_test.distribution_sold) as b on a.iso_yr_wk_nbr=b.iso_yr_wk_nbr group by state, cal_yr_mo_nbr ,brewer, segment")
# 
# bir_coverage=read.csv('bir_iri_manufac.csv')
# 
# mapping=data.frame(segment= c('FMB','VALUE','CORE','Premium','NON ALC','CORE PLUS'),
#                    new=c('FMB','VALUE','CORE','H.E','N.A','Premium'))
# 
# bir_coverage$grouping='cover'
# names(bir_coverage)[names(bir_coverage)=='IRI']='brewer'
# 
# mult=merge(mult,bir_coverage[,c('brewer','grouping')],by='brewer',all.x = T)
# mult$grouping[is.na(mult$grouping)]='n_cover'
# 
# mult=data.table(mult)[,.(vol=sum(vol_cases)),by=.(state_cd,cal_yr_mo_nbr,segment,grouping)]
# mult=merge(mult,mapping,by='segment',all.x = T)
# mult$segment=NULL
# names(mult)[names(mult)=='new']='segment'
# mult=mult[!is.na(segment)]
# mult2=mult[grouping=='cover']
# names(mult2)[names(mult2)=='vol']='vol_c'
# mult2$grouping=NULL
# mult=merge(mult[grouping=='n_cover',.(state_cd,cal_yr_mo_nbr,vol,segment)],mult2,by=c('state_cd','cal_yr_mo_nbr','segment'),all=T)
# mult[is.na(vol),vol:=0]
# mult[is.na(vol_c),vol_c:=0]
# mult$multiplier=(mult$vol + mult$vol_c)/mult$vol_c
# mult[vol_c==0,multiplier:=1]
# roof=quantile(mult$multiplier,0.995)
# mult[multiplier>roof,multiplier:=roof]
# 
# 
# tot_abi=data.table(vol_dat)[vol_sales>0,.(ab_vol=sum(vol_sales)),by=.(state_cd,rtlr_num,cal_yr_mo_nbr)]
# iri_impte=merge(iri_impte,tot_abi,by=c('rtlr_num','cal_yr_mo_nbr'))
# iri_impte[,comp_tot:=sum(imp_comp_share),by=.(rtlr_num,cal_yr_mo_nbr)]
# iri_impte[,tot_vol:=ab_vol/(1- comp_tot)]
# iri_impte[comp_tot==1,tot_vol:=1]
# iri_impte[,comp_vol:=tot_vol*imp_comp_share]
# iri_impte=merge(iri_impte,mult[,.(state_cd,cal_yr_mo_nbr,segment,multiplier)],by=c('state_cd','segment','cal_yr_mo_nbr'),all.x=T)
# iri_impte[,mean_m:=median(multiplier,na.rm=T),by=.(state_cd,segment)]
# iri_impte[is.na(multiplier),multiplier:=mean_m]
# iri_impte[is.na(multiplier),multiplier:=1]
# iri_impte[,comp_vol:=comp_vol*multiplier]
# iri_impte[,comp_tot:=sum(comp_vol),by=.(rtlr_num,cal_yr_mo_nbr)]
# iri_impte[,tot_vol:=comp_tot+ab_vol]
# iri_impte[,comp_sh_adj:=comp_vol/tot_vol]
# iri_impte=iri_impte[,.(rtlr_num,segment,cal_yr_mo_nbr,comp_sh_adj)]
# names(iri_impte)[names(iri_impte)=='comp_sh_adj']='imp_comp_share'
# 
# mult[,zip_s:='MAINSTREAM']
# mult[segment=='H.E',zip_s:='CRAFT']
# mult[segment=='N.A',zip_s:='NON-ALC']
# mult[segment=='VALUE',zip_s:='ECONOMY']
# mult[segment=='FMB',zip_s:='FMB']
# 
# mult=mult[,.(vol=sum(vol),vol_c=sum(vol_c)),by=.(state_cd,cal_yr_mo_nbr,zip_s)]
# mult$multiplier=(mult$vol + mult$vol_c)/mult$vol_c
# mult[vol_c==0,multiplier:=1]
# roof=quantile(mult$multiplier,0.995)
# mult[multiplier>roof,multiplier:=roof]
# names(mult)[names(mult)=='zip_s']='segment'
# 
...
# 
# 
# zip=merge(zip,unique(data.table(vol_dat)[,.(zip,state_cd)]),by=c('zip'))
# zip=merge(zip,mult[,.(state_cd,cal_yr_mo_nbr,segment,multiplier)],by=c('state_cd','cal_yr_mo_nbr','segment'),all.x=T,sort = F)
# zip=data.table(zip)
# zip[,mean_m:=median(multiplier,na.rm=T),by=.(state_cd,segment)]
# zip[is.na(multiplier),multiplier:=mean_m]
# zip[is.na(multiplier),multiplier:=1]
# zip[,ind:=ind*multiplier]
# zip[,state_cd:=NULL]
# zip[,multiplier:=NULL]
# zip[,mean_m:=NULL]
# zip=data.frame(zip)
...
load('rf_pkg.RData')
beta1=read.csv('pkg_reg2.csv')
beta1=beta1$V1
month_c=read.csv('month_cols_pkg.csv')
